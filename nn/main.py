...
def padding_resize():
    ...
    for namee in os.listdir(directory1):
        ...
        for str1 in os.listdir(directory):
            ...
            if w > h:
                h1 = w - h
                img1 = cv2.copyMakeBorder(img, 0, 0, int(
                    h1/2), int(h1/2), cv2.BORDER_CONSTANT, None, value=255)
                cv2.imwrite('hhd_dataset_after\\'+namee+'\\'+str1, img1)
            if w < h:
                w1 = h - w
                img1 = cv2.copyMakeBorder(
                    img, int(w1 / 2), int(w1 / 2), 0, 0, cv2.BORDER_CONSTANT, None, value=255)
                cv2.imwrite('hhd_dataset_after\\'+namee + '\\'+str1, img1)
            if w == h:
                cv2.imwrite('hhd_dataset_after\\'+namee + '\\'+str1, img1)
        ...
        for str1 in os.listdir(directory):
            ...
            cv2.imwrite('hhd_dataset_finale\\'+namee + '\\' + str1, 255-img1)
